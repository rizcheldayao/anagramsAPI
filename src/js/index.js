  ...
  document.getElementById('button').addEventListener('click', (e) => {
    if (document.getElementById('select').value !== 'addWord') {
      ...
      if (document.getElementById('select').value === 'deleteAll') {
        ...
        const deleteWord = document.getElementById('input').value;
        xhttp.open(
          'DELETE',
          `http://localhost:3000/words/${deleteWord}.json`,
          true,
        );
        xhttp.send();
      } else if (document.getElementById('select').value === 'fetchAnagram') {
        const anagram = document.getElementById('input').value;
        const limit = document.getElementById('limit').value;
        xhttp.open(
          'GET',
          `http://localhost:3000/anagrams/${anagram}.json?limit=${limit}`,
          true,
        );
        xhttp.send();
      }
